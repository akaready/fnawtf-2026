...
import { X, ExternalLink, Check, Home, Hand, GitBranch, Calendar, Play, DollarSign, Eye, EyeOff, ChevronDown, Copy, Loader2, GripVertical, Clapperboard } from 'lucide-react';
...
import { deleteProposal, updateProposal, createProposalVersion, type ClientRow } from '@/app/admin/actions';
...
export const ProposalAdminEditor = forwardRef<ProposalEditorHandle, Props>(function ProposalAdminEditor({
  ...
  onSwitchVersion, onNewVersionCreated,
}, editorRef) {
  ...
  const handleDirty = useCallback(() => {
    ...
    // Auto-transition from generated → draft on first edit
    if (status === 'generated') {
      setStatus('draft');
      void updateProposal(proposal.id, { status: 'draft' });
  ...
  const handleStatusChange = useCallback(async (newStatus: 'draft' | 'sent' | 'archived') => {
    setStatus(newStatus);
    setStatusOpen(false);
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {proposal.generated_by && status === 'generated' && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-admin-full bg-admin-accent-bg text-accent text-admin-sm flex-shrink-0">
                  🤖 Generated
                </span>
              )}
            </div>
            <p className="text-sm mt-1 text-admin-text-muted font-mono truncate">
              /p/{proposal.slug}/v{proposal.version_number}
            </p>
          </div>
      ...
      <div className="flex-1 min-h-0 relative">
        ...
        <div className={activeTab === 'welcome' ? 'h-full overflow-hidden' : 'hidden'}>
          <WelcomeTab
            ...
            snippets={snippets}
            onDirty={handleDirty}
            generatedBy={proposal.generated_by}
          ...
          <>
            ...
            <div ref={statusRef} className="relative">
              <button
                ...
                className={`${status === 'archived' ? 'btn-secondary opacity-60' : status !== 'draft' ? 'btn-success' : 'btn-secondary'} gap-1.5 px-4 py-2.5 text-sm font-medium`}
              >
                {status === 'draft' ? 'Draft' : status === 'archived' ? 'Archived' : 'Sent'}
                  ...
                  <div className="absolute bottom-full mb-1 left-0 min-w-[160px] bg-admin-bg-overlay border border-admin-border rounded-lg shadow-xl py-1 z-50">
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-xs flex items-center justify-between transition-colors ${
                        status === 'sent' || status === 'viewed' || status === 'accepted' ? 'text-admin-success bg-admin-success-bg/30' : 'text-admin-text-muted hover:bg-admin-bg-hover'
                      }`}
                    >
                      ...
                      {(status === 'sent' || status === 'viewed' || status === 'accepted') && <Check size={12} />}
                    </button>
                    <button
                      type="button"
                    ...
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-xs flex items-center justify-between transition-colors ${
                        status === 'archived' ? 'text-admin-text-primary bg-admin-bg-active' : 'text-admin-text-muted hover:bg-admin-bg-hover'
                      }`}
                    >
                      <span className="flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-admin-text-faint opacity-40" />
                        Archive
                      </span>
                      {status === 'archived' && <Check size={12} />}
                    </button>
                  </div>
                </>
              )}
            ...
            <a
              href={`/p/${proposal.slug}/v${proposal.version_number}?pwd=${proposal.proposal_password}`}
              target="_blank"
            ...
            {onDuplicate && (
              <button
                ...
                className="btn-secondary px-4 py-2.5 text-sm"
                title="Duplicate as a new proposal (new client)"
              >
            ...
            {onNewVersionCreated && (
              <button
                type="button"
                onClick={async () => {
                  const newId = await createProposalVersion(proposal.id);
                  onNewVersionCreated(newId);
                }}
                className="btn-secondary px-4 py-2.5 text-sm"
                title="Create a new version of this proposal (same client)"
              >
                <GitBranch size={13} />
                New Version
              </button>
