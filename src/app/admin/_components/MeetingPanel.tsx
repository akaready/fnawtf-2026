...
const inputCls =
  'w-full rounded-lg border border-border/40 bg-black/50 px-3 py-2 text-sm text-muted-foreground placeholder:text-muted-foreground/30 focus:outline-none focus:ring-1 focus:ring-white/20';
    ...
    <PanelDrawer open={open} onClose={onClose} width="w-[540px]">
      ...
      <div className="flex items-center gap-4 px-6 pt-5 pb-4 border-b border-[#2a2a2a]">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1.5 text-sm text-muted-foreground/50">
            <span>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden admin-scrollbar">
        {tab === 'details' && (
          <div className="px-6 py-5 space-y-5">
            {/* ── Relationships ── */}
            <div>
              <label className="text-xs text-muted-foreground/30 uppercase tracking-wider font-medium">
                Relationships
              </label>
                ...
                <div className="mt-2 space-y-2">
                  {meeting.meeting_relationships.map((rel) => (
                    ...
                    >
                      ...
                      {rel.matched_email && (
                        <span className="text-[10px] text-muted-foreground/20 flex-shrink-0 hidden group-hover:inline">
                          {rel.matched_email}
                        </span>
                      )}
                      <button
                        ...
                        disabled={linking}
                        className="w-6 h-6 flex items-center justify-center rounded text-muted-foreground/0 group-hover:text-muted-foreground/30 hover:!text-red-400 hover:!bg-red-500/10 transition-colors flex-shrink-0 cursor-pointer"
                      >
                        <Unlink size={14} />
                      </button>
                    </div>
            ...
            {/* ── Attendees ── */}
            <div>
              <label className="text-xs text-muted-foreground/30 uppercase tracking-wider font-medium flex items-center gap-1.5">
                <Users size={12} />
            ...
            {meeting.description && (
              <div>
                ...
                <p className="mt-1.5 text-sm text-muted-foreground/40 whitespace-pre-wrap leading-relaxed break-words">
                  {meeting.description}
                </p>
              </div>
            )}
