...
export function ScriptSceneHeader({ scene, scriptGroupId, locations = [], onUpdate, onDelete, editing, onEditingChange, onGenerate, generating, isGenerating }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className={`group/actions flex items-center gap-0.5 flex-shrink-0 rounded-admin-sm transition-all ${confirmDelete ? 'opacity-100' : 'opacity-0 group-hover/scene:opacity-100'} hover:bg-admin-bg-hover`}>
        {confirmDelete ? (
          <>
            <button
